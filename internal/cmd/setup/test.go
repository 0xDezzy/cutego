...
import (
	...
	"github.com/0xDezzy/cutego/internal/cmd/deploy"
...
func Test(target string, docker, vagrant bool, vagrantsystem string) {
	...
	for cat, list := range examples {
		for _, example := range list {
			...
			path := filepath.Join(strings.TrimSpace(utils.GoListOptional("{{.Dir}}", "github.com/0xDezzy/cutego/internal/examples", "-find", "get doc dir")), example)
