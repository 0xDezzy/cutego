...
func GetImports(path, target, tagsCustom string, level int, onlyDirect bool) []string {
	...
	if onlyDirect {
		...
		defer func() {
			...
			if importsQmlOrQuick == "" {
				...
				stdImport[len(stdImport)-1] = "github.com/0xDezzy/cutego/internal/binding/runtime"

				cmd := utils.GoList(fmt.Sprintf("{{if not .Standard}}{{if eq .ImportPath \"%v\"}}{{else}}{{range .Imports}}{{if eq . \"github.com/0xDezzy/cutego/qml\" \"github.com/0xDezzy/cutego/quick\"}}{{.}}{{end}}{{end}}{{end}}{{end}}", strings.Join(stdImport, "\" \"")), "-deps", utils.BuildTags(tags))
			...
			if importsInterop == "" {
				...
				cmd := utils.GoList(fmt.Sprintf("{{if not .Standard}}{{if eq .ImportPath \"%v\"}}{{else}}{{range .Imports}}{{if eq . \"github.com/0xDezzy/cutego/interop\"}}{{.}}{{end}}{{end}}{{end}}{{end}}", strings.Join(stdImport, "\" \"")), "-deps", utils.BuildTags(tags))
				if !utils.UseGOMOD(path) || (utils.UseGOMOD(path) && !strings.Contains(strings.Replace(path, "\\", "/", -1), "/vendor/")) {
					cmd.Dir = path
				} else if utils.UseGOMOD(path) && strings.Contains(strings.Replace(path, "\\", "/", -1), "/vendor/") {
					cmd.Dir = filepath.Dir(utils.GOMOD(path))
			...
			if importsFlutter == "" {
				...
				cmd := utils.GoList(fmt.Sprintf("{{if not .Standard}}{{if eq .ImportPath \"%v\"}}{{else}}{{range .Imports}}{{if eq . \"github.com/0xDezzy/cutego/flutter\"}}{{.}}{{end}}{{end}}{{end}}{{end}}", strings.Join(stdImport, "\" \"")), "-deps", utils.BuildTags(tags))
				if !utils.UseGOMOD(path) || (utils.UseGOMOD(path) && !strings.Contains(strings.Replace(path, "\\", "/", -1), "/vendor/")) {
					cmd.Dir = path
				} else if utils.UseGOMOD(path) && strings.Contains(strings.Replace(path, "\\", "/", -1), "/vendor/") {
					cmd.Dir = filepath.Dir(utils.GOMOD(path))
	...
	for _, l := range libs {
		...
		go func(l string) {
			...
			if strings.Contains(l, "github.com/0xDezzy/cutego") && !(strings.Contains(l, "qt/internal") || strings.Contains(l, "qt/flutter") || strings.Contains(l, "qt/interop")) {
				if strings.Contains(l, "github.com/0xDezzy/cutego/") {
					importedStdMutex.Lock()
...
func GetQtStdImports() (o []string) {
	importedStdMutex.Lock()
	for k := range importedStd {
		o = append(o, strings.TrimPrefix(k, "github.com/0xDezzy/cutego/"))
	}
	importedStdMutex.Unlock()
	return
}
...
func QtModVersion(path string) string {
	cmd := exec.Command("go", "list", "-m", "-f", "{{.Version}}", "github.com/0xDezzy/cutego")
	cmd.Dir = path
...
func RestartWithPinnedVersion(path string) bool {
	...
	cmd = exec.Command("go", "install", "-v", "-tags=no_env", "github.com/0xDezzy/cutego/cmd/...")
	cmd.Dir = path
